...
# ORS (Django) Baota install helper
# - Create/prepare venv
...
# - Install gunicorn and print a recommended start command

ORS_DIR="${ORS_DIR:-/www/wwwroot/ors}"
VENV_DIR="${VENV_DIR:-.venv}"
PYTHON_BIN="${PYTHON_BIN:-python3}"

# Gunicorn defaults for Baota reverse-proxy usage
BIND_HOST="${BIND_HOST:-127.0.0.1}"
BIND_PORT="${BIND_PORT:-9001}"
GUNICORN_WORKERS="${GUNICORN_WORKERS:-3}"

# Bootstrap defaults (used only when root/admin/dept are created)
...
random_password() {
  # Use python stdlib to avoid relying on openssl
  "$PYTHON_BIN" - <<'PY'
import secrets
...
main() {
  ...
  # If caller passed DB_* / DJANGO_* env vars at runtime, write them into .env
  # so Django settings/env("...") can pick them up during migrate.
  set_kv_in_env() {
    key="$1"
    val="$2"
    # Rewrite whole file (small .env expected); this avoids replacement-string escaping issues.
    tmp="$(mktemp)"
    awk -v k="$key" -v v="$val" '
      BEGIN{found=0}
  ...
  maybe_set_from_runtime_env() {
    ...
    if [ -n "$value" ]; then
      ...
      # Avoid leaking secrets into Baota logs
      masked="$value"
      case "$key" in
        *PASSWORD*|*SECRET*|*KEY*)
          masked="******"
  ...
  # Make sure we have a real secret key (avoid insecure default)
  if grep -qE '^DJANGO_SECRET_KEY=' .env; then
    current_secret="$(grep -E '^DJANGO_SECRET_KEY=' .env | tail -n 1 | cut -d'=' -f2-)"
    if [ "$current_secret" = "change-me" ] || [ -z "$current_secret" ]; then
      new_secret="$(random_password)"
      # replace the line in-place (POSIX-compatible fallback)
      if command -v perl >/dev/null 2>&1; then
        perl -pi -e "s/^DJANGO_SECRET_KEY=.*/DJANGO_SECRET_KEY=$new_secret/" .env
      else
        # Fallback: rewrite file (still safe for small .env)
        tmp="$(mktemp)"
  ...
  # Bootstrap root/admin/dept.
  # Passwords should be provided via env vars; if absent, generate random and print.
